...
export default function ClockInOut({ attendance, onClockIn, onClockOut, loading }) {
  ...
  return <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 via-blue-800 to-slate-950 p-6 text-white shadow-xl sm:p-8"><div className="flex flex-col justify-between gap-8 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2 text-sm font-semibold text-blue-200"><FiClock /> Live attendance clock</div><p className="mt-4 text-5xl font-bold tracking-tight">{now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p><p className="mt-2 text-blue-100">{now.toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })}</p></div><div className="sm:text-right"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold capitalize ${status === 'late' ? 'bg-amber-400/20 text-amber-200' : status === 'present' ? 'bg-emerald-400/20 text-emerald-200' : 'bg-white/10 text-blue-100'}`}>{status}</span><p className="mt-3 text-sm text-blue-100">{attendance?.clockIn ? `Clocked in at ${new Date(attendance.clockIn).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'You have not clocked in today'}</p><button className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-white px-5 py-3 font-semibold text-blue-800 transition hover:bg-blue-50 disabled:cursor-wait disabled:opacity-60 sm:w-auto" disabled={loading} onClick={isClockedIn ? onClockOut : onClockIn} type="button"><FiMapPin />{loading ? 'Getting location...' : isClockedIn ? 'Clock Out' : 'Clock In'}</button></div></div></section>
}
